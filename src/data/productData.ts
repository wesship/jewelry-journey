...
// Enhanced product data with detailed descriptions and images for product detail page
...
export const getProductById = (id: string) => {
  ...
  if (id in productDetails) {
    return productDetails[id as keyof typeof productDetails];
  }
  
  // Otherwise return basic product info from allProducts
  const product = allProducts.find(product => product.id === id);
  
  if (!product) {
    // If product is not found, return a default product or throw an error
    return {
      id: "not-found",
